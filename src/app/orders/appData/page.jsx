...
const appData = () => {
  return (
    <div className="ml-[340px] mt-[75px] p-6 min-h-[calc(100vh-75px)] mr-[50px] bg-[#F2F4F7] rounded-[20px] mb-[50px]">
      ...
      <h2 className="text-[32px] font-semibold text-[#1570EF] mb-1">Application Data</h2>
      <p className=" text-[#1A1A1A] mb-6">These leads are through the Loomiz App</p>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <Card title="Quote received" count="8" route="/products/addproduct"  />
          <Card title="Orders approved" count="16" />
          <Card title="Orders Rejected" count="9" />
        </div>
      </div>

      {/* Queries Section */}
      <div className="mb-6">
        <h3 className="text-[20px] font-[Smedium]  font-semibold text-[#0E1C2F] mb-3">Queries</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <Card title="Queries received" count="8"  route="/orders/appData/QueriesReceived"/>
          <Card title="Queries approved" count="16" />
          <Card title="Queries Rejected" count="9" />
        </div>
      </div>
...
const Card = ({ title, count, route }) => (
    <div className="bg-white rounded-lg p-5 shadow-sm flex justify-between items-start">
      ...
      {route ? (
        <Link href={route}>
          <button className="text-[#194185] text-sm font-medium cursor-pointer">View all</button>
        </Link>
      ) : (
        <button className="text-[#194185] text-sm font-medium cursor-pointer  opacity-50">
          View all
        </button>
      )}
    </div>
