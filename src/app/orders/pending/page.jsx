...
export default function PendingOrders() {
  ...
  useEffect(() => {
    const fetchPendingQuotes = async () => {
      try {
        setIsLoading(true);
        const response = await fetch('/api/orders/pending-quotes');
    ...
    return (
      <div className="ml-[340px] mt-[75px] p-8 min-h-[calc(100vh-75px)] mr-[50px] bg-[#F8F9FB] rounded-lg mb-[50px]">
        <div className="bg-white p-8 rounded-lg shadow-sm text-center flex flex-col items-center justify-center min-h-[200px]">
          ...
          <p className="text-red-500 mt-4 text-lg">Error loading pending RFQs</p>
  ...
  return <RFQReceived initialQuotes={quotes} initialStats={stats} />;
}