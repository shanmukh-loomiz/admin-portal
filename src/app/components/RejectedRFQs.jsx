...
const RejectedRFQs = ({ initialQuotes = [], initialStats = { total: 0, pending: 0, accepted: 0, rejected: 0 } }) => {
  const router = useRouter();
  const [rejectedQuotes, setRejectedQuotes] = useState(initialQuotes);
    ...
    <div className="ml-[340px] mt-[75px] p-8 min-h-[calc(100vh-75px)] mr-[50px] bg-[#F8F9FB] rounded-lg mb-[50px]">
      ...
      <div className="flex items-start justify-between mb-10">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-2xl font-semibold text-gray-800">Rejected RFQs</h2>
        </div>
      ...
      {rejectedQuotes.length === 0 ? (
        <div className="bg-white p-8 rounded-lg shadow-sm mt-6 text-center flex flex-col items-center justify-center min-h-[200px]">
          ...
          <p className="text-gray-500 mt-4 text-lg">No rejected RFQs found</p>
          <p className="text-gray-400 text-sm mt-2">All your rejected requests for quotes will appear here</p>
        </div>
      ) : (
        rejectedQuotes.map((quote, idx) => {
